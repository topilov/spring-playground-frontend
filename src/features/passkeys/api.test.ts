...
describe('passkeys api', () => {
  ...
  it('loads the authenticated user passkeys', async () => {
    ...
    expect(url).toBe('http://localhost:8080/api/auth/passkeys');
  ...
  it('starts authenticated passkey registration', async () => {
    ...
    const [url, init] = fetchMock.mock.calls[0];
    expect(url).toBe('http://localhost:8080/api/auth/passkeys/register/options');
  ...
  it('verifies authenticated passkey registration', async () => {
    ...
    const [url, init] = fetchMock.mock.calls[0];
    expect(url).toBe('http://localhost:8080/api/auth/passkeys/register/verify');
    expect(init?.method).toBe('POST');
    expect(init?.credentials).toBe('include');
  ...
  it('renames a passkey', async () => {
    ...
    const [url, init] = fetchMock.mock.calls[0];
    expect(url).toBe('http://localhost:8080/api/auth/passkeys/10');
  ...
  it('starts unauthenticated passkey login', async () => {
    ...
    const [url, init] = fetchMock.mock.calls[0];
    expect(url).toBe('http://localhost:8080/api/auth/passkey-login/options');
    expect(init?.method).toBe('POST');
    expect(init?.credentials).toBe('include');
  ...
  it('verifies passkey login and returns the normal session user', async () => {
    ...
    const [url, init] = fetchMock.mock.calls[0];
    expect(url).toBe('http://localhost:8080/api/auth/passkey-login/verify');
    expect(init?.method).toBe('POST');
    expect(init?.credentials).toBe('include');
    expect(init?.body).toBe(
      JSON.stringify({
